...
- (void)viewDidLoad {
    ...
    self.title = QLS(@"title_update_phone");
    ...
    self.oldPhoneTextField.placeholder = QLS(@"placeholder_current_phone");
    ...
    self.oldSmsTextField.placeholder = QLS(@"placeholder_current_phone_code");
    ...
    [oldSmsButton setTitle:QLS(@"btn_get_sms_code") forState:UIControlStateNormal];
    ...
    self.phoneTextField.placeholder = QLS(@"placeholder_new_phone");
    ...
    self.smsTextField.placeholder = QLS(@"placeholder_new_phone_code");
...
- (void)sureButtonClick {
    ...
    [[QuecUserService sharedInstance] updatePhone:self.phoneTextField.text newInternationalCode:@"86" newPhoneCode:self.smsTextField.text oldPhone:self.oldPhoneTextField.text oldInternationalCode:@"86" oldPhoneCode:self.oldSmsTextField.text success:^{
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        [self.view makeToast:QLS(@"msg_phone_updated") duration:3 position:CSToastPositionCenter];
    } failure:^(NSError *error) {
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        [self.view makeToast:error.localizedDescription duration:3 position:CSToastPositionCenter];
    }];
